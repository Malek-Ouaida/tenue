...
def register(payload: RegisterIn, req: Request, db: Session = Depends(get_db)) -> LoginOut:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail={"error": e.code})
    except service.AuthError:
        # no email enumeration
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail={"error": "registration_failed"})
...
def login(payload: LoginIn, req: Request, db: Session = Depends(get_db)) -> LoginOut:
    ip = _client_ip(req) or "unknown"
    ua = _user_agent(req)
...
def refresh(payload: RefreshIn, req: Request, db: Session = Depends(get_db)) -> AuthTokensOut:
    ip = _client_ip(req) or "unknown"
    ua = _user_agent(req)
...
def logout(session_id: UUID = Depends(require_session_id), db: Session = Depends(get_db)) -> OkOut:
    service.logout(db=db, session_id=session_id)
    return OkOut(ok=True)


@router.post("/logout-all", response_model=OkOut)
def logout_all(user: User = Depends(require_user), db: Session = Depends(get_db)) -> OkOut:
